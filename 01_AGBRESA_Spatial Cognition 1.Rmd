...
```
#### Table 1. Data collection summary Spatial Cognition 1.
```{r echo = FALSE}
datatable(spat_bat_1_summary) %>%
    formatRound(columns=c("Completed (%)"), digits = 1)

round(mean(spat_bat_1_summary$'Completed (%)'), 1)
```
...
# Drop vars
desc <- subset(desc, variable != "direction_z")
...
p.sut_desc <- func_plot_line_facet(desc %>%
                                     mutate(time.f = fct_relevel(time.f, "BDC-3", "HDT2", "HDT30", "HDT59", "R+12"),
                                            variable = fct_relevel(variable, "rt_z","variable_pointing_error_z", "eff_z"),
...
```
#### Table 2. Mixed Model Summary for Spatial Updating Task.
```{r echo = FALSE}
# Print table
...
```
#### Table 3. Mixed Model Group Contrasts for Spatial Updating Task.
```{r echo = FALSE}
# Print table 
...
```
#### Table 4. Mixed Model Summary testing the Main Effect of Group for Spatial Updating Task.
```{r echo = FALSE}
# Print table
...
# Compute desc
desc <- func_desc(df_pto, group_3, time.f, cond.f, variable)
...
p.pto_desc <- func_plot_line_facet(desc %>%
                                     mutate(time.f = fct_relevel(time.f, "BDC-3", "HDT2", "HDT30", "HDT59", "R+12"),
                                            variable = fct_relevel(variable, "rt_z", "variable_pointing_error_z", "eff_z", "dist_error_z"),
                                            group_3 = fct_relevel(group_3, "CTRL", "cAG", "iAG")))
...
```
#### Table 6. Mixed Model Summary for Point to Oriign Task.
```{r echo = FALSE}
# Print table
...
tbl_contr_emm_fac_df$Time <- rep ( c("HDT2", rep("", tmp_n_time),
                                   ...
                                   length(dvList) )
```
#### Table 7. Mixed Model Group Contrasts for Point to Oriign Task.
```{r echo = FALSE}
# Table for contrasts
...
```
#### Table 8. Mixed Model Summary testing the Main Effect of Group for Point to Oriign Task.
```{r echo = FALSE}
# Print ANOVA table
...
```
#### Table 9. Mixed Model Contrasts Comparing Groups for Point to Oriign Task.
```{r echo = FALSE}
...
# Combine plot
p.pto_comb <- plot_grid(p.pto_desc, p.pto_emmeans_main + rremove("x.text") + theme( strip.text.y = element_blank() ),
          labels = c("a", "b"),
          ncol = 2, nrow = 1,
          rel_widths = c(2.2, 0.8))
...
dev.off()
```
#### Figure 2. Point to Origin Task: (a) Time course of speed, precision, and efficiency before, during, and after bed rest. (b) Main effect of bed rest. Data are means and SD adjusted for baseline.
```{r, echo = FALSE, fig.width = 10, fig.height = 8, fig.cap = ""}
print(p.pto_comb)
```
# Navigation Search Task 2D
...
# Drop vars
desc <- subset(desc, variable != "total_target_visits_z" &
...
# Plot desc 
# New facet label names for variable
new_labels <- c("Speed","Accuracy","Distance","Revisits")
...
```
#### Table 10. Mixed Model Summary for Navigation Search Task 2D.
```{r echo = FALSE}
# Print table
...
#table_contrasts_emm_fac_by_time_df$Contrast <- rep(c("cAG vs. CTRL","iAG vs. CTRL", "cAG vs. iAG"), length(dvList))
```
#### Table 11. Mixed Model Group Contrasts for Navigation Search Task 2D.
```{r echo = FALSE}
# Table for contrasts
...
```
#### Table 12. Mixed Model Summary testing the Main Effect of Group for Navigation Search Task 2D.
```{r echo = FALSE}
# Print ANOVA table
...
```
#### Table 13. Mixed Model Contrasts Comparing Groups for Navigation Search Task 2D.
```{r echo = FALSE}
# Print Contrast table
...
dev.off()
```
#### Figure 3. Navigation Search Task 2D: (a) Time course of speed, precision, and efficiency before, during, and after bed rest. (b) Main effect of bed rest. Data are means and SD adjusted for baseline.
```{r, echo = FALSE, fig.width = 9, fig.height = 9, fig.cap = ""}
print(p.navis_2d_comb)
```
...
```
#### Table 14. Mixed Model Summary for Four Mountains Task.
```{r echo = FALSE}
# Print table
...
```
#### Table 15. Mixed Model Group Contrasts for Four Mountains Task.
```{r echo = FALSE}
# Print table
...
```
#### Table 16. Mixed Model Summary testing the Main Effect of Group for Four Mountains Task.
```{r echo = FALSE}
# Print ANOVA table
...
```
#### Table 17. Mixed Model Contrasts Comparing Groups for Four Mountains Task.
```{r echo = FALSE}
# Print Contrast table
...
dev.off()
```
#### Figure 4. Fourn Mountains Task: (a) Time course of speed, precision, and efficiency before, during, and after bed rest. (b) Main effect of bed rest. Data are means and SD adjusted for baseline.
```{r, echo = FALSE, fig.width = 10, fig.height = 3.5, fig.cap = ""}
print(p.4mtn_comb)
```